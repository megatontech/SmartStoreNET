...
namespace SmartStore.Web.Framework.UI
{
    public static class ScaffoldExtensions
    {
        ...
        public static string DeclarationProduct<T>(this HtmlHelper<T> helper, string id, string name, string typeName = "ProductTypeName", string typeLabelHint = "ProductTypeLabelHint")
        {
            string namePart = null;

            if (id.HasValue())
            {
                string url = UrlHelper.GenerateContentUrl("~/Admin/DeclarationProduct/Edit/", helper.ViewContext.RequestContext.HttpContext);
                namePart = "<a href=\"{0}<#= {1} #>\"><#= {2} #></a>".FormatInvariant(url, id, name);
            }
            else
            {
        ...
        public static HelperResult DeclarationProduct<T>(this HtmlHelper<T> helper, int id, string name, string typeName, string typeLabelHint)
        {
            if (id == 0 && name.IsEmpty())
                return null;
