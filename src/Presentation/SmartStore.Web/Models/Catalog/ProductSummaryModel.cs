...
namespace SmartStore.Web.Models.Catalog
{
	...
	public partial class DeclarationProductSummaryModel : ModelBase, IListActions, IDisposable
	{
		public static DeclarationProductSummaryModel Empty = new DeclarationProductSummaryModel(new PagedList<DeclarationProduct>(new List<DeclarationProduct>(), 0, int.MaxValue));

		public DeclarationProductSummaryModel(IPagedList<DeclarationProduct> products)
		{
			Guard.NotNull(products, nameof(products));
		...
		public class SummaryItem : EntityModelBase
		{
			private readonly WeakReference<DeclarationProductSummaryModel> _parent;

			public SummaryItem(DeclarationProductSummaryModel parent)
			{
				//Parent = parent;
				_parent = new WeakReference<DeclarationProductSummaryModel>(parent);
				...
				Picture = new PictureModel();
				Attributes = new List<Attribute>();
				SpecificationAttributes = new List<DeclarationProductSpecificationModel>();
				Badges = new List<Badge>();
			}

			public DeclarationProductSummaryModel Parent
			{
				get
				{
					_parent.TryGetTarget(out var parent);
			...
			// important. According to it's importance, show attribute value in grid or list mode.
			// E.g. perfect for "Energy label" > "EEK A++", or special material (e.g. "Leather") etc.
			public IList<DeclarationProductSpecificationModel> SpecificationAttributes { get; set; }
			public IList<ColorAttributeValue> ColorAttributes { get; set; }
			public IList<Badge> Badges { get; set; }
		}
